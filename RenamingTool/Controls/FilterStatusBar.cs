...
namespace RenamingTool.Controls
{
    /// <summary>    
    ...
    public class FilterStatusBar : Control
    {
        ...
        public override void OnApplyTemplate()
        {
            ...
            UpdateFilterStatusBarVisibility(false);
            ClearFilterButton = GetTemplateChild("PART_CloseButton") as Button;
            Textblock = GetTemplateChild("PART_TextBlock") as TextBlock;
            Tooltip = GetTemplateChild("PART_ToolTip") as ToolTip;
            UnWireEvents();
            WireEvents();
        }
                ...
                if (item != null && item.FilterPredicates.Count > 0)
                {
                    foreach (var filterItem in item.FilterPredicates)
                    {
                        if (filters == "")
                            ...
                            filters += " " + GetPredicateType(filterItem.PredicateType) + " " + GetFilterInfo(filterItem, item);
                    }
                }
            }
            UpdateFilterText(filters);
            UpdateFilterStatusBarVisibility(filters.Trim() != "");
        }
        ...
        protected virtual string GetFilterInfo(FilterPredicate filter, GridColumn visColumn)
        {
            ...
            if (filtervalue != null)
            {
                if (visColumn.CellType == "DateTime")
                    filtervalue = DateTimeFormatString(visColumn as GridDateTimeColumn, Convert.ToDateTime(filtervalue));
        ...
        {
            Loaded += OnFilterStatusBarLoaded;
